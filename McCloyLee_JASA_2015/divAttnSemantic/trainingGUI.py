...
# semantic categories
body = ['arm', 'chin', 'foot', 'leg', 'mouth', 'nose']
fooddrink = ['beer', 'bread', 'meat', 'rice', 'soup', 'wine']
furniture = ['bed', 'chair', 'couch', 'desk', 'lamp', 'rug']
plants = ['bark', 'grass', 'leaf', 'root', 'stick', 'tree']
animals = ['bird', 'cat', 'cow', 'mouse', 'pig', 'snake']
weather = ['breeze', 'cloud', 'fog', 'rain', 'sky', 'wind']
clothing = ['belt', 'dress', 'hat', 'scarf', 'shirt', 'suit']
allwords = body + fooddrink + furniture + plants + animals + weather + clothing
allwords = sorted(allwords)
cats = dict(body=body, fooddrink=fooddrink, furniture=furniture, plants=plants,
            animals=animals, weather=weather, clothing=clothing)
...
def on_pick(event):
    ...
    if len(word_x) == len(allwords):
        for key, val in word_x.iteritems():
            ...
            elif key in fooddrink:
                word_correct[key] = int(round(val)) == 5
            elif key in clothing:
                word_correct[key] = int(round(val)) == 6
            elif key in furniture:
                word_correct[key] = int(round(val)) == 7
